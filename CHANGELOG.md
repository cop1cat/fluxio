...
- Public `Pipeline` API with `invoke`, `stream`, `run_step`, `replay`, `diff`, and
  `explain` methods, plus `async with Pipeline(...)` for deterministic thread-pool
  shutdown.
...
- `Parallel([...])` blocks with `PARALLEL` and `FIRE_FORGET` modes, plus
...
- Durable execution with `CheckpointStore`: `InMemoryStore`, `RedisStore`.
  ...
  default. `CheckpointVersionError` on pipeline version mismatch and
...
- Testing utilities: `StepHarness` for isolated stage execution (with sync and
  async context-manager support) and `make_ctx` fixture helper.
