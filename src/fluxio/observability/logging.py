...
class LoggingCallback(BaseCallback):
    ...
    async def on_pipeline_end(self, run_id: str, ctx: Context, duration_ms: int) -> None:
        _logger.info("pipeline_end run_id=%s duration_ms=%d", run_id, duration_ms)
    ...
    async def on_step_stream(self, run_id: str, step: str, chunk: Any) -> None:
        if self.chunk_logging:
            _logger.debug("step_stream run_id=%s step=%s chunk=%r", run_id, step, chunk)
    ...
    async def on_checkpoint(self, run_id: str, step: str, ip: int) -> None:
        _logger.debug("checkpoint run_id=%s step=%s ip=%d", run_id, step, ip)
