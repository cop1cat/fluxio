...
Перед каждым вызовом стейджа интерпретатор сохраняет `ctx.snapshot()` вместе с указателем на текущую инструкцию. Если стейдж бросает исключение, последний успешный чекпоинт остаётся нетронутым и исключение пробрасывается дальше — `resume=True` стартует именно с этого чекпоинта и повторяет упавший стейдж заново.
